...
def generate_regime_data(n: int=500, regime_probs: tuple[float, float]=(0.7, 0.3), stds: tuple[float, float]=(1, 5), seed: int=42) -> pd.DataFrame:
...
def fit_markov_switching(data: np.ndarray, k_regimes: int=2, switching_variance: bool=True) -> Any:
    """Fit Markov switching regression model."""
    model = MarkovRegression(data, k_regimes=k_regimes, trend='c', switching_variance=switching_variance)
    return model.fit()
...
def calculate_regime_statistics(df: pd.DataFrame) -> dict[int, dict[str, float]]:
    ...
    for regime in [0, 1]:
        ...
        stats_dict[regime] = {'mean': regime_data.mean(), 'std': regime_data.std(), 'skewness': stats.skew(regime_data), 'kurtosis': stats.kurtosis(regime_data)}
    return stats_dict
...
def calculate_regime_durations(df: pd.DataFrame) -> dict[int, float]:
    ...
    durations = {}
    for regime in [0, 1]:
        regime_runs = (df['Predicted_Regime'] == regime).astype(int).groupby((df['Predicted_Regime'] != df['Predicted_Regime'].shift()).cumsum()).sum()
...
def plot_regime_data(df: pd.DataFrame, output_path: Path, plot: bool=False):
    ...
    for regime in [0, 1]:
        ...
        ax.scatter(df[mask]['Time'], df[mask]['Data'], label=f'Regime {regime}', color=color, alpha=0.6, s=20)
    ax.set_xlabel('Time')
    ax.set_ylabel('Value')
    ax.legend(loc='best')
...
def plot_regime_comparison(df: pd.DataFrame, output_path: Path, plot: bool=False):
    ...
    if not plot:
        return
    fig, ax = plt.subplots(figsize=(12, 6))
    ax.plot(df['Time'], df['True_Regime'], label='True Regime', color='#4A90A4', linewidth=1.2, alpha=0.7)
    ax.plot(df['Time'], df['Predicted_Regime'], label='Predicted Regime', color='#D4A574', linewidth=1.2, alpha=0.7)
    ax.set_xlabel('Time')
...
def plot_density_distribution(df: pd.DataFrame, output_path: Path, plot: bool=False):
    ...
    for regime in [0, 1]:
        color = '#4A90A4' if regime == 0 else '#D4A574'
        sns.kdeplot(data=df[df['True_Regime'] == regime]['Data'], label=f'Regime {regime}', color=color, ax=ax)
    ax.set_xlabel('Value')
...
def plot_transition_matrix(result: Any, output_path: Path, plot: bool=False):
    ...
    sns.heatmap(transition_matrix, annot=True, cmap='coolwarm', center=0.5, square=True, linewidths=0.5, cbar_kws={'shrink': 0.8}, ax=ax)
...
def plot_confusion_matrix(df: pd.DataFrame, output_path: Path, plot: bool=False):
    ...
    sns.heatmap(confusion_matrix, annot=True, fmt='d', cmap='Blues', square=True, linewidths=0.5, cbar_kws={'shrink': 0.8}, ax=ax)
